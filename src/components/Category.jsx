...
const Category = ({setRemoveVideoResponseFromCategory,removeVideoResponseFromView}) => {
  ...
  const videoDropOverCategory = async (e,categoryId)=>{
    ...
    // console.log(data);
    //get  detailas dropping category and insert videoDetails of category allvideos
    const selectedCategory = allCategories?.find(item=>item.id==categoryId)
    selectedCategory.allVideos.push(data)
    console.log(selectedCategory);
    //update selected category into json file - call api
    await updateCategoryAPI(categoryId,selectedCategory)
    //remove  video from all videos - call api
      ...
      <div className="container-fluid mt-3">
       {
        ...
        allCategories?.map(categoryDetails=>(
          <div key={categoryDetails?.id}  droppable="true" onDragOver={dragOverCategory} onDrop={e=>videoDropOverCategory(e,categoryDetails?.id)} className="border rounded p-3 mb-2">
        ...
        :
        <div className="text-danfer fw-bolder">No Categories added yet!!</div>
       }
      </div>
      <Modal
        show={show}
